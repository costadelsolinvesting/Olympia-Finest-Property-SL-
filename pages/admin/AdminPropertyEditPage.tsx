...
    formData.append("form-name", "property-submission"); // REQUIRED for Netlify to catch the form
    formData.append("propertyData", JSON.stringify(propertyToSave));
    formData.append("isNew", isNew.toString());

    try {
        // Netlify form submission requires POST to the root path /
...
    } catch (error) {
        console.error('Erreur lors de l\'envoi du formulaire à Netlify:', error);
        alert('Erreur lors de l\'enregistrement. Vérifiez la console pour les détails. Le formulaire a peut-être été soumis correctement à Netlify Forms.');
    }
    
...
        {/* Netlify hidden form field to capture the submission */}
